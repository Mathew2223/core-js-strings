...
function isPalindrome(str) {
  const replace = str.replace(/[^a-zA-Z0-9]/g, '').toLowerCase();
...
function extractNameFromTemplate(value) {
  const match = value.match(/\b(\w+\s+\w+)\b/);
  if (match) {
    return match[0];
  }
  return '';
...
function encodeToRot13(str) {
  return str
    ...
    .map((item) => {
      ...
      if (char >= 65 && char <= 90) {
        return String.fromCharCode(((char - 65 + 13) % 26) + 65);
      }
      if (char >= 97 && char <= 122) {
        return String.fromCharCode(((char - 97 + 13) % 26) + 97);
      }
      return item;
    })
    .join('');
